...
const update_quotation = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    quotation_service_1.default.updateQuotation({status: 'Finally'}, quotationId)
    .then((id) => {
        return res.status(200).json({
            success: true,
            errorCode: 0,
            message: "Quotation atualizado com sucesso."
        });
    })
    .catch(err => {
    if (err.name === 'MongoServerError' && err.code === 11000) {
        ...
        return res.status(500).json({ success: false, errorCode: err.code, message: "Problema ao atualizar Quotation" });
    }
});
});
